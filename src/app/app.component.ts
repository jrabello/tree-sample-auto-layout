...
export class AppComponent {
    ...
    public data: Object = {
        ...
        doBinding: (nodeModel: NodeModel, data: object, diagram: Diagram) => {
            nodeModel.shape = {
                ...
                type: 'HTML',  content: this.renderHTMLElement(),//`<div style="color: #ffffff">html-test</div>`,
    ...
    renderHTMLElement(): HTMLElement {
      const htmlString = `<div style="color: #ffffff">html-test</div>`
      const element = this.createElementFromHTML(htmlString)
        ...
        if (target.className === 'image-pattern-style') {
            switch (target.id) {
                ...
                case 'pattern4':
                    ...
                    break;
                case 'pattern5':
                    subTreeOrientation = 'Vertical'; // fazer isso aqui
                    subTreeAlignment = 'Right';
                    break;
                case 'pattern6':
            ...
            this.diagram.layout.getLayoutInfo = (node: NodeModel, options: TreeInfo) => {
                const nodeData = (node.data as DataInfo)
                console.log({node, options, nodeData, nodeDataRole: nodeData['Role']});

                // if (target.id === 'pattern4' || target.id === 'pattern3') {
                //     options.offset = -50;
                // }

                // if (nodeData['Role'] === 'General Manager') {
                //     console.log({children: options.children[0]});
                //     options.assistants.push(options.children[0]);
                //     options.children.splice(0, 1);
                // }
                // console.log({options});
                ...
                if (nodeData['Role'] === 'Marketing Manager') {
                    console.log({subTreeOrientation, subTreeAlignment, role: nodeData['Role']});
